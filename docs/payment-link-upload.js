(function() {
  ...
  dialog.innerHTML = '<header><div><h2 id="paymentLinkTitle">결제링크 업로드</h2><p>결제내역을 대조하고 새 수납만 입력합니다.</p></div><button type="button" data-close aria-label="팝업 닫기">✕</button></header>' +
    ...
    '<section data-pane="upload"><label class="payment-link-file">결제내역 엑셀 선택<input type="file" accept=".xlsx" aria-label="결제내역 엑셀 파일"></label><p class="payment-link-help">학생·귀속 월을 확인한 뒤 중복 검사를 실행하세요. 취소 건은 원결제와 환불을 확인한 후 수납 관리에서 처리합니다.</p><div class="payment-link-table" data-preview></div><footer><button type="button" data-check disabled>중복 검사</button><button type="button" data-save disabled>새 수납 입력</button></footer></section>' +
  ...
  var rows = [], roster = [], busy = false, events = [], fileName = '', monthAtOpen = '';
  ...
  function render() {
    ...
    $('[data-preview]').innerHTML = rows.length ? '<table><thead><tr><th>원본 이름 / 품목</th><th>학생</th><th>귀속 월</th><th>결제일 / 승인번호</th><th>금액</th><th>대조 결과</th></tr></thead><tbody>' + rows.map(function(r,i) {
      return '<tr><td>' + esc(r.rawName) + '<small>' + esc(r.itemName) + '</small></td><td><select aria-label="' + r.rowNumber + '행 학생" data-row="' + i + '" data-field="studentName"' + (r.issue ? ' disabled' : '') + '>' + options(names,r.studentName) + '</select></td><td><select aria-label="' + r.rowNumber + '행 귀속 월" data-row="' + i + '" data-field="monthName"' + (r.issue ? ' disabled' : '') + '>' + options(state.tuition.months || [monthAtOpen],r.monthName) + '</select></td><td>' + esc(r.paidAt) + '<small>' + esc(r.approvalNo) + '</small></td><td class="payment-link-money">' + won(-r.amount) + '</td><td>' + esc(r.issue || r.message || (!eligible(r) ? '학생·월 확인 필요' : '검사 전')) + '</td></tr>';
    }).join('') + '</tbody></table>' : '';
  }
  ...
  async function process(save) {
    ...
    try {
      ...
      message('입력 완료 ' + (counts.saved || 0) + '건 · 중복 제외 ' + (counts.duplicate || 0) + '건 · 입력 가능 ' + (counts.ready || 0) + '건 · 확인 필요 ' + ((counts.error || 0) + (counts.review || 0)) + '건');
      if (save) refreshTuitionSummary(true);
  ...
  $('input[type=file]').addEventListener('change', async function(e) {
    var file = e.target.files[0]; if (!file) return;
    rows = []; setBusy(true); message('엑셀을 읽는 중입니다.');
    try {
  ...
  $('[data-preview]').addEventListener('change', function(e) {
    ...
    var r = rows[Number(el.dataset.row)]; r[el.dataset.field] = el.value; r.result = ''; r.message = ''; setBusy(false);
  });
