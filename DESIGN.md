...
- Supply inventory matrix: one shared item owns one stable row; `본관`, `2관`, and `3관` each have a named stock cell with current quantity, safety/maximum reference, and 40px increment/decrement controls. The aggregate status names the affected branches, the header stays sticky inside the table scroll region, and only this matrix may scroll horizontally at narrow widths.
