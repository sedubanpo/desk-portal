...
- Supply inventory matrix: one shared item owns one stable row; `본관`, `2관`, and `3관` each have a named stock cell with current quantity, safety/maximum reference, and compact increment/decrement controls. Branch identity is persistent across the register, sticky table header, stock cells, and edit fieldsets: forest tint for `본관`, cool blue for `2관`, and warm amber for `3관`. Color always appears with the branch name, never as the only identifier.
- Inventory state uses Lucide icon plus semantic color and text: `package-x`/red for `재고 없음`, `triangle-alert`/amber for `보충 필요`, and `circle-check`/green for `안정 재고`. Status treatments are inline labels, not rounded badges.
