...
- `--ds-bg-app` `#F7F8FC`: application background.
- `--ds-bg-panel` `#FFFFFF`: primary working surface.
- `--ds-bg-subtle` `#F5F6FB`: quiet secondary surface.
- `--ds-bg-warm` `#FAF9F5`: low-alert shared work surface.
- `--ds-fg` `#293449`: main text.
- `--ds-fg-soft` `#48556B`: dense body text.
- `--ds-muted` `#5E6B80`: secondary text.
- `--ds-primary` `#4F5FC4`: primary action and active state.
- `--ds-primary-strong` `#3D4BA4`: selected navigation state.
- `--ds-on-primary` `#FFFFFF`: text on primary actions.
- `--ds-border` `#E3E7EF`: default hairline.
- `--ds-border-soft` `#EEF0F5`: quiet separators.
- `--ds-ring` `rgba(79, 95, 196, 0.18)`: focus ring.
...
- Page title: 22-25px, weight 650, line-height 1.2, letter-spacing 0.
- Section title: 16-18px, weight 650, line-height 1.3, letter-spacing 0.
- Dense label: 11-13px, weight 600-650, line-height 1.25.
- Body: 13-14px, weight 400-550, line-height 1.45-1.6.
- Numeric cells: tabular numerals, weight 600-650.
...
- Tuition table density: the last-contact column stays at 136px, editable amount columns preserve a one-line numeric value, and body copy uses the dense 600-650 weight range for consistent scanning.
...
- Tuition workbook export: the selected month exports as one workbook with a summary, a student reconciliation sheet, and a transaction ledger. The toolbar command stays compact; workbook amounts use numeric cells, transaction signs are translated into received/refunded columns, and the student sheet exposes both internal and Access comparison differences.
...
- Segmented controls: same height across items, active state uses quiet indigo surface, inactive state remains white.
...
- Supply search is the primary inventory lookup control. It uses the primary border, a visible focus ring, and a concrete light placeholder such as `예: A4, 종이컵, 프린터 토너`.
...
## Payroll calendar and time detail (2026-09-09)
The existing workspace CSS remains authoritative. Calendar dates lead with 15px bold numerals, compact hours, tabular financial values and subdued working-day surfaces. Selection uses the existing olive primary and light lime surface. The chart uses lime gross-sales bars, olive net-sales lines and the existing dashed hour series; tooltips retain exact won values. Summary rows align numbers right. Hourly detail reserves two columns for the time axis and overlapping classes, with gaps spanning a separate row below. Flat semantic event surfaces replace gradients and shadows. First/last time labels remain visible, and the axis extends to the latest ending class. Calendar selection supports Enter/Space with restored focus. Financial formulas and persistence remain unchanged.
