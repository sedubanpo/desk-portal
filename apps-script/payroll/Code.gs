...
const TUITION_PAYMENT_READ_INDEX_FIRESTORE_COLLECTION = "tuitionPaymentReadIndexes";
const TUITION_MONTHLY_READ_INDEX_FIRESTORE_COLLECTION = "tuitionMonthlyReadIndexes";
...
function firestoreRequestWithServiceAccount_(method, path, query, payload) {
  ...
  for (var attempt = 1; attempt <= attempts; attempt++) {
    try {
      ...
      lastCode = code;
      lastText = text;
      if (attempt >= attempts || !isTransientFirestoreStatus_(code)) break;
    } catch (fetchError) {
      lastText = fetchError && fetchError.message ? fetchError.message : String(fetchError);
      if (attempt >= attempts) break;
    }
    Utilities.sleep(250 * attempt);
  }
  throw new Error("Firestore 요청 실패(" + lastCode + ") " + cleanPath + ": " + lastText);
}

function isTransientFirestoreStatus_(code) {
  var n = parseInt(code || 0, 10) || 0;
...
function getTuitionBootstrapData() {
  try {
    ...
    var summary = getTuitionMonthSummaryFastSnapshot_(selectedMonth, months);
    if (!summary || !summary.success) {
      summary = buildPendingTuitionMonthSummary_(selectedMonth, months, "summary-snapshot-missing");
    }
    return {
      success: true,
      months: months,
...
function buildPendingTuitionMonthSummary_(monthName, months, reason) {
  var stats = buildTuitionSummaryStats_([], []);
  return {
    success: true,
    selectedMonth: String(monthName || "").trim(),
    ...
    rows: [],
    pendingSummary: true,
    pendingReason: String(reason || "pending-summary"),
    cache: { source: "bootstrap-pending" },
    indexStatus: buildTuitionIndexStatus_(monthName, { source: "bootstrap-pending" })
  };
}

function getTuitionMonthSummaryFastSnapshot_(monthName, months) {
  var month = String(monthName || "").trim();
  if (!month) return null;
  var summaryCacheKey = getTuitionSummaryCacheKey_(month);
  var cachedSummary = readTuitionJsonCache_(summaryCacheKey);
  if (cachedSummary && cachedSummary.success) {
    cachedSummary.months = Array.isArray(months) ? months.slice() : (cachedSummary.months || []);
    cachedSummary.selectedMonth = month;
    cachedSummary.cache = {
      source: "script-cache",
      key: summaryCacheKey
    };
    cachedSummary.indexStatus = buildTuitionIndexStatus_(month, cachedSummary.cache);
    return cachedSummary;
  }
  var firestoreSnapshot = readTuitionMonthSnapshotFromFirestore_(month);
  if (firestoreSnapshot && firestoreSnapshot.success) {
    firestoreSnapshot.months = Array.isArray(months) ? months.slice() : (firestoreSnapshot.months || []);
    firestoreSnapshot.selectedMonth = month;
    firestoreSnapshot.cache = {
      source: "firestore-snapshot",
      documentId: buildTuitionMonthSnapshotDocId_(month),
      computedAt: firestoreSnapshot.snapshot && firestoreSnapshot.snapshot.computedAt || ""
    };
    writeTuitionJsonCache_(summaryCacheKey, firestoreSnapshot, TUITION_MONTH_SUMMARY_CACHE_TTL_SECONDS);
    firestoreSnapshot.indexStatus = buildTuitionIndexStatus_(month, firestoreSnapshot.cache);
...
function invalidateTuitionScriptCacheOnly_(monthName) {
  var month = String(monthName || "").trim();
  if (!month) return;
  try {
...
function updateTuitionMonthSnapshotAfterPaymentWrite_(monthName, paymentRecord) {
  var month = String(monthName || "").trim();
  if (!month || !paymentRecord) return false;
  ...
  var alreadyApplied = snapshot.allPayments.some(hasPayment) || snapshot.payments.some(hasPayment);
  if (!snapshot.allPayments.some(hasPayment)) snapshot.allPayments.push(payment);
  if (!snapshot.payments.some(hasPayment)) snapshot.payments.push(payment);
  ...
  var paidAmount = 0 - toPayrollNumber_(payment.amount);
  snapshot.rows = snapshot.rows.map(function(row) {
    ...
    copy.collectedAmount = Math.round(toPayrollNumber_(copy.collectedAmount) + paidAmount);
    ...
    if (
      (toPayrollNumber_(copy.guideAmount) > 0 && toPayrollNumber_(copy.collectedAmount) >= toPayrollNumber_(copy.guideAmount)) ||
      (toPayrollNumber_(copy.guideAmount) <= 0 && toPayrollNumber_(copy.collectedAmount) > 0)
    ) {
      copy.unpaidStatus = "납부완료";
    } else if (toPayrollNumber_(copy.guideAmount) > 0 && toPayrollNumber_(copy.collectedAmount) > 0 && copy.outstandingAmount > 0) {
      copy.unpaidStatus = "일부완료";
    }
    return copy;
  });
...
function enrichTuitionSummaryRecentPayments_(summary, months, selectedMonth, fallbackRows) {
  ...
  var recentRows = loadSeededTuitionRecentPaymentIndexRows_();
...
function getTuitionMonthSummary(payload) {
  try {
    ...
    if (req.syncPortalPayments === true) {
    ...
    if (!forceRefresh && monthName) {
      ...
      if (firestoreSnapshot && firestoreSnapshot.success) {
        ...
        seedTuitionReadIndexesFromSnapshot_(monthName, firestoreSnapshot);
        firestoreSnapshot.cache = {
          source: "firestore-snapshot",
    ...
    var recentIndexBundle = readTuitionRecentPaymentIndexBundle_();
    var recentPaymentRows = recentIndexBundle.seeded ? recentIndexBundle.rows : [];
    if (!recentPaymentRows.length) {
      recentPaymentRows = mergeTuitionPaymentRows_(allPaymentRows, getAllTuitionPaymentRows_(months, { allowSheetFallback: false }));
      seedTuitionPaymentReadIndexes_(recentPaymentRows);
    }
    ...
    var result = {
      ...
      rows: list,
      indexStatus: buildTuitionIndexStatus_(monthName, { source: "computed" })
    };
    if (canUseSummaryCache) {
...
function writeTuitionFollowupToFirestore_(record) {
  ...
  updateTuitionFollowupMonthIndex_(row);
  invalidateTuitionFollowupReportIndexes_(row.studentName);
  return { success: true, id: docId };
}

function normalizeTuitionFollowupIndexRows_(rows) {
  return (Array.isArray(rows) ? rows : []).map(normalizeTuitionFollowupRecord_).filter(function(row) {
    return !!row;
  });
}

function readTuitionFollowupMonthIndexBundle_(monthName) {
  var safeMonth = normalizeTuitionMonthName_(monthName);
  var docId = buildTuitionMonthlyReadIndexDocId_("followups", safeMonth);
  if (!docId) return { rows: [], seeded: false, updatedAt: "" };
  try {
    var doc = firestoreGetDocument_(TUITION_MONTHLY_READ_INDEX_FIRESTORE_COLLECTION, docId);
    if (!doc) return { rows: [], seeded: false, updatedAt: "" };
    return {
      rows: normalizeTuitionFollowupIndexRows_(doc.rows).filter(function(row) {
        return row.monthName === safeMonth;
      }),
...
function writeTuitionFollowupMonthIndex_(monthName, rows, seeded) {
  var safeMonth = normalizeTuitionMonthName_(monthName);
  var docId = buildTuitionMonthlyReadIndexDocId_("followups", safeMonth);
  if (!docId) return false;
  var normalized = normalizeTuitionFollowupIndexRows_(rows).filter(function(row) {
    return row.monthName === safeMonth;
  });
  try {
    firestoreSetDocument_(TUITION_MONTHLY_READ_INDEX_FIRESTORE_COLLECTION, docId, {
      monthName: safeMonth,
      rows: normalized,
      seeded: seeded === true,
...
function updateTuitionFollowupMonthIndex_(record) {
  var row = normalizeTuitionFollowupRecord_(record);
  if (!row || !row.monthName) return false;
  try {
    var bundle = readTuitionFollowupMonthIndexBundle_(row.monthName);
    var rows = mergeTuitionFollowupRows_(bundle.rows, [row]);
    return writeTuitionFollowupMonthIndex_(row.monthName, rows, bundle.seeded === true);
  } catch (e) {
    return false;
  }
}
...
function writeTuitionPaymentToFirestore_(record) {
  ...
  invalidateTuitionPaymentReportIndexes_(row.studentName);
  return { success: true, id: docId };
}

function getTuitionPaymentReadIndexUpdatedAt_() {
  try {
...
function buildTuitionMonthlyReadIndexDocId_(kind, monthName) {
  var safeKind = String(kind || "").trim().replace(/[^\w-]/g, "_");
  var safeMonth = normalizeTuitionMonthName_(monthName);
  if (!safeKind || !safeMonth) return "";
  return safeKind + "_" + safeMonth.replace(/[^\w-]/g, "_");
}

function buildTuitionReportReadIndexDocId_(kind, key) {
  var safeKind = String(kind || "").trim().replace(/[^\w-]/g, "_");
  var rawKey = String(key || "global").trim();
  if (!safeKind || !rawKey) return "";
  var bytes = Utilities.newBlob(rawKey).getBytes();
  return "report_" + safeKind + "_" + Utilities.base64EncodeWebSafe(bytes).replace(/=+$/g, "").slice(0, 90);
}

function readTuitionReportReadIndex_(kind, key, expectedVersion) {
  var docId = buildTuitionReportReadIndexDocId_(kind, key);
  if (!docId) return null;
  try {
    var doc = firestoreGetDocument_(TUITION_MONTHLY_READ_INDEX_FIRESTORE_COLLECTION, docId);
    if (!doc || doc.kind !== String(kind || "").trim()) return null;
    if (expectedVersion && String(doc.version || "") !== String(expectedVersion)) return null;
    if (!doc.payload || doc.payload.success !== true) return null;
    var payload = doc.payload;
    payload.cache = {
      source: "firestore-report-index",
      documentId: docId,
      updatedAt: String(doc.updatedAt || "")
    };
    return payload;
...
function writeTuitionReportReadIndex_(kind, key, payload, version) {
  var docId = buildTuitionReportReadIndexDocId_(kind, key);
  if (!docId || !payload || payload.success !== true) return false;
  try {
    firestoreSetDocument_(TUITION_MONTHLY_READ_INDEX_FIRESTORE_COLLECTION, docId, {
...
function deleteTuitionReportReadIndex_(kind, key) {
  var docId = buildTuitionReportReadIndexDocId_(kind, key);
  if (!docId) return false;
  try {
    firestoreDeleteDocument_(TUITION_MONTHLY_READ_INDEX_FIRESTORE_COLLECTION, docId);
    return true;
  } catch (e) {
    return false;
  }
...
function buildTuitionReportMonthsVersion_(months) {
  return sortTuitionMonthNames_(months || []).join("|");
}

function invalidateTuitionPaymentReportIndexes_(studentName) {
  deleteTuitionReportReadIndex_("monthly_sales", "global");
  var key = normalizeTuitionStudentName_(studentName);
  if (key) deleteTuitionReportReadIndex_("student_history", key);
}

function invalidateTuitionFollowupReportIndexes_(studentName) {
  deleteTuitionReportReadIndex_("guide_dashboard", "global");
  var key = normalizeTuitionStudentName_(studentName);
  if (key) deleteTuitionReportReadIndex_("student_history", key);
}

function compactTuitionIndexPart_(label, bundle) {
  var rows = bundle && Array.isArray(bundle.rows) ? bundle.rows : [];
  return {
    label: label,
    ready: !!(bundle && bundle.seeded),
    count: rows.length,
    updatedAt: String(bundle && bundle.updatedAt || "")
  };
}

function buildTuitionIndexStatus_(monthName, summaryCache) {
  var safeMonth = normalizeTuitionMonthName_(monthName);
  var payment = safeMonth ? readTuitionMonthPaymentIndexBundle_(safeMonth) : { rows: [], seeded: false, updatedAt: "" };
  var followups = safeMonth ? readTuitionFollowupMonthIndexBundle_(safeMonth) : { rows: [], seeded: false, updatedAt: "" };
  var charges = safeMonth ? readTuitionMonthChargeIndexBundle_(safeMonth) : { rows: [], seeded: false, updatedAt: "" };
  var recent = readTuitionRecentPaymentIndexBundle_();
  return {
    monthName: safeMonth,
    checkedAt: getTuitionPaymentReadIndexUpdatedAt_(),
    summary: {
      source: String(summaryCache && summaryCache.source || "computed"),
      updatedAt: String(summaryCache && (summaryCache.computedAt || summaryCache.updatedAt) || "")
    },
    recentPayments: compactTuitionIndexPart_("최근 수납", recent),
    monthPayments: compactTuitionIndexPart_("월별 수납", payment),
    followups: compactTuitionIndexPart_("안내/연락", followups),
    charges: compactTuitionIndexPart_("안내금액", charges)
  };
}

function seedTuitionReadIndexesFromSnapshot_(monthName, summary) {
  var safeMonth = normalizeTuitionMonthName_(monthName);
  if (!safeMonth || !summary || summary.success !== true) return false;
  var indexStatus = buildTuitionIndexStatus_(safeMonth, { source: "firestore-snapshot" });
  var snapshotRows = Array.isArray(summary.rows) ? summary.rows : [];
  var allPayments = Array.isArray(summary.allPayments) ? summary.allPayments : [];
  var monthPayments = Array.isArray(summary.payments) ? summary.payments : [];
  var wrote = false;

  if (!indexStatus.recentPayments.ready) {
    wrote = writeTuitionRecentPaymentIndex_(allPayments, true) || wrote;
  }
  if (!indexStatus.monthPayments.ready) {
    wrote = writeTuitionMonthPaymentIndex_(safeMonth, monthPayments, true) || wrote;
  }
  if (!indexStatus.followups.ready) {
    var followupRows = snapshotRows.map(function(row) {
      return {
        monthName: safeMonth,
        studentName: row.studentName,
        guideAmount: row.guideAmount,
    ...
    wrote = writeTuitionFollowupMonthIndex_(safeMonth, followupRows, true) || wrote;
  }
  if (!indexStatus.charges.ready) {
    var chargeRows = snapshotRows.map(function(row) {
      return {
        monthName: safeMonth,
        ...
        guideAmount: row.guideAmount,
        active: true,
        source: "tuition_month_snapshot",
        updatedAt: row.lastUpdatedAt || ""
      };
    });
    wrote = writeTuitionMonthChargeIndex_(safeMonth, chargeRows, true) || wrote;
  }
  return wrote;
}

function normalizeTuitionPaymentIndexRows_(rows) {
  return (Array.isArray(rows) ? rows : []).map(normalizeTuitionPaymentRecord_).filter(function(row) {
    return !!row;
  }).sort(compareTuitionPaymentRowsDesc_);
}

function readTuitionRecentPaymentIndexBundle_() {
  try {
    var doc = firestoreGetDocument_(TUITION_PAYMENT_READ_INDEX_FIRESTORE_COLLECTION, TUITION_RECENT_PAYMENT_INDEX_DOC_ID);
    if (!doc) return { rows: [], seeded: false, updatedAt: "" };
    return {
      rows: normalizeTuitionPaymentIndexRows_(doc.payments),
      seeded: doc.seeded === true,
      updatedAt: String(doc.updatedAt || "")
    };
  } catch (e) {
...
function loadSeededTuitionRecentPaymentIndexRows_() {
  var bundle = readTuitionRecentPaymentIndexBundle_();
  return bundle.seeded ? bundle.rows : [];
}

function writeTuitionRecentPaymentIndex_(rows, seeded) {
  var normalized = normalizeTuitionPaymentIndexRows_(rows).slice(0, TUITION_RECENT_PAYMENT_INDEX_LIMIT);
  if (!normalized.length && seeded !== true) return false;
  try {
    firestoreSetDocument_(TUITION_PAYMENT_READ_INDEX_FIRESTORE_COLLECTION, TUITION_RECENT_PAYMENT_INDEX_DOC_ID, {
      payments: normalized,
      seeded: seeded === true,
      updatedAt: getTuitionPaymentReadIndexUpdatedAt_(),
      source: "desk_portal"
...
function getTuitionPaymentPaidDateKey_(row) {
  var baseMonth = getTuitionPaymentBaseMonthName_(row);
  var raw = String(row && (row.paidAt || row.inputAt) || "").trim();
  var ms = parseTuitionDateTimeMs_(raw, baseMonth);
  if (ms < 0) ms = parseTuitionDateTimeMs_(row && row.inputAt, baseMonth);
  if (ms < 0) return "";
  var date = new Date(ms);
  return date.getUTCFullYear() + "-" + payrollPad2_(date.getUTCMonth() + 1) + "-" + payrollPad2_(date.getUTCDate());
}

function buildTuitionDailyPaymentIndexDocId_(dateKey) {
  var safeKey = String(dateKey || "").trim();
  if (!/^\d{4}-\d{2}-\d{2}$/.test(safeKey)) return "";
  return "daily_" + safeKey.replace(/-/g, "_");
}

function readTuitionDailyPaymentIndexRows_(dateKey) {
  var docId = buildTuitionDailyPaymentIndexDocId_(dateKey);
  if (!docId) return [];
  try {
    var doc = firestoreGetDocument_(TUITION_PAYMENT_READ_INDEX_FIRESTORE_COLLECTION, docId);
    return normalizeTuitionPaymentIndexRows_(doc && doc.payments);
  } catch (e) {
    return [];
  }
}

function writeTuitionDailyPaymentIndex_(dateKey, rows) {
  var docId = buildTuitionDailyPaymentIndexDocId_(dateKey);
  if (!docId) return false;
  var normalized = normalizeTuitionPaymentIndexRows_(rows).slice(0, TUITION_DAILY_PAYMENT_INDEX_LIMIT);
...
function hasTuitionPaymentRequestInFirestore_(requestId) {
  return !!loadTuitionPaymentRequestFromFirestore_(requestId);
}

function loadTuitionPaymentRequestFromFirestore_(requestId) {
  var id = normalizeTuitionClientRequestId_(requestId);
  if (!id) return null;
  try {
    var doc = firestoreGetDocument_(TUITION_PAYMENT_FIRESTORE_COLLECTION, buildTuitionPaymentFirestoreDocId_({ requestId: id }));
    var row = normalizeTuitionPaymentRecord_(doc);
    return row && normalizeTuitionClientRequestId_(row.requestId) === id ? row : null;
  } catch (e) {
    return null;
  }
}
...
function writeTuitionMonthChargeToFirestore_(record) {
  ...
  updateTuitionMonthChargeIndex_(row);
  deleteTuitionReportReadIndex_("student_history", row.studentName);
  return { success: true, id: docId };
}

function normalizeTuitionMonthChargeIndexRows_(rows) {
  return (Array.isArray(rows) ? rows : []).map(normalizeTuitionMonthChargeRecord_).filter(function(row) {
    return !!row && row.active !== false;
  });
}

function readTuitionMonthChargeIndexBundle_(monthName) {
  var safeMonth = normalizeTuitionMonthName_(monthName);
  var docId = buildTuitionMonthlyReadIndexDocId_("charges", safeMonth);
  if (!docId) return { rows: [], seeded: false, updatedAt: "" };
  try {
    var doc = firestoreGetDocument_(TUITION_MONTHLY_READ_INDEX_FIRESTORE_COLLECTION, docId);
    if (!doc) return { rows: [], seeded: false, updatedAt: "" };
    return {
      rows: normalizeTuitionMonthChargeIndexRows_(doc.rows).filter(function(row) {
        return row.monthName === safeMonth;
      }),
      seeded: doc.seeded === true,
      updatedAt: String(doc.updatedAt || "")
...
function writeTuitionMonthChargeIndex_(monthName, rows, seeded) {
  var safeMonth = normalizeTuitionMonthName_(monthName);
  var docId = buildTuitionMonthlyReadIndexDocId_("charges", safeMonth);
  if (!docId) return false;
  var normalized = normalizeTuitionMonthChargeIndexRows_(rows).filter(function(row) {
    return row.monthName === safeMonth;
  });
  try {
    firestoreSetDocument_(TUITION_MONTHLY_READ_INDEX_FIRESTORE_COLLECTION, docId, {
...
function mergeTuitionMonthChargeRows_(primaryRows, secondaryRows) {
  var map = {};
  function add(record) {
    var row = normalizeTuitionMonthChargeRecord_(record);
    if (!row) return;
    var key = row.monthName + "|" + normalizeTuitionStudentName_(row.studentName);
    map[key] = row;
  }
  (primaryRows || []).forEach(add);
  (secondaryRows || []).forEach(add);
...
function updateTuitionMonthChargeIndex_(record) {
  var row = normalizeTuitionMonthChargeRecord_(record);
  if (!row || !row.monthName) return false;
  try {
    var bundle = readTuitionMonthChargeIndexBundle_(row.monthName);
    var rows = mergeTuitionMonthChargeRows_(bundle.rows, [row]);
    return writeTuitionMonthChargeIndex_(row.monthName, rows, bundle.seeded === true);
  } catch (e) {
    return false;
  }
}
...
function loadTuitionMonthChargeRowsFromFirestore_(monthName) {
  var safeMonth = String(monthName || "").trim();
  if (safeMonth) {
    var indexed = readTuitionMonthChargeIndexBundle_(safeMonth);
    if (indexed.seeded) return indexed.rows;
  }
  try {
    var rows = firestoreListCollection_(TUITION_MONTH_CHARGE_FIRESTORE_COLLECTION, 1000)
      .map(normalizeTuitionMonthChargeRecord_)
      .filter(function(row) {
        return !!row && row.active !== false && (!safeMonth || row.monthName === safeMonth);
      });
    if (safeMonth) writeTuitionMonthChargeIndex_(safeMonth, rows, true);
    return rows;
  } catch (e) {
    return null;
  }
...
function getTuitionPaymentRowsForMonth_(monthName, options) {
  ...
  if (isTuitionPaymentMonthFirestoreReady_(safeMonth)) {
    if (Array.isArray(firestoreRows)) {
      writeTuitionMonthPaymentIndex_(safeMonth, firestoreRows, true);
...
function getTuitionPortalPaymentRowsFromSheet_(monthName) {
  ...
  parseTuitionRows_(portalSheet).forEach(function(row) {
    ...
    if (safeMonth && copy.sourceDueMonth !== safeMonth && copy.sourceMonth !== safeMonth) return;
    rows.push(copy);
  });
  rows.sort(compareTuitionPaymentRowsDesc_);
  return rows;
...
function syncMissingTuitionPortalPaymentsToFirestore_(monthName) {
  var safeMonth = String(monthName || "").trim();
  var stats = { success: true, monthName: safeMonth, total: 0, written: 0, skipped: 0, errors: [] };
  if (!safeMonth) return stats;
  var portalRows = getTuitionPortalPaymentRowsFromSheet_(safeMonth);
  stats.total = portalRows.length;
  if (!portalRows.length) return stats;
  var firestoreRows = loadTuitionPaymentRowsFromFirestore_(safeMonth);
  if (!Array.isArray(firestoreRows)) {
    stats.success = false;
    stats.errors.push({ stage: "read", message: "Firestore 수강료 수납 조회 실패" });
    return stats;
  }
  var existingMap = {};
...
function getTuitionPaymentRowsForMonthFromSheet_(monthName) {
  ...
  if (portalRows.length) {
    rows = mergeTuitionPaymentRows_(rows, portalRows);
  }
...
function getAllTuitionPaymentRows_(months, options) {
  ...
  getTuitionPortalPaymentRowsFromSheet_("").forEach(function(row) {
    var sourceMonth = String(row.sourceMonth || "").trim();
    var dueMonth = String(row.sourceDueMonth || parseTuitionDueMonthName_(row.dueDate)).trim();
    if ((sourceMonth && monthMap[sourceMonth]) || (dueMonth && monthMap[dueMonth])) {
      portalCandidateRows.push(row);
      rows.push(row);
    }
  });

  if (opts.allowSheetFallback === false) {
    return mergeTuitionPaymentRows_(portalCandidateRows, firestoreCandidateRows);
  }
...
function getTuitionPaymentRowsForMonthsFromIndexes_(months, options) {
  var opts = options || {};
  var monthList = sortTuitionMonthNames_(months || []);
  var indexedRows = [];
  var missingMonths = [];
  monthList.forEach(function(monthName) {
    var bundle = readTuitionMonthPaymentIndexBundle_(monthName);
    if (bundle.seeded) {
      indexedRows = mergeTuitionPaymentRows_(indexedRows, bundle.rows);
    } else {
      missingMonths.push(monthName);
    }
  });
  if (!missingMonths.length || opts.indexOnly === true) return indexedRows;
  return mergeTuitionPaymentRows_(indexedRows, getAllTuitionPaymentRows_(missingMonths, opts));
}

function getTuitionFollowupRowsForMonthsFromIndexes_(months, options) {
  var opts = options || {};
  var monthList = sortTuitionMonthNames_(months || []);
  var rows = [];
  var missingMonths = [];
  monthList.forEach(function(monthName) {
    var bundle = readTuitionFollowupMonthIndexBundle_(monthName);
    if (bundle.seeded) {
      rows = mergeTuitionFollowupRows_(rows, bundle.rows);
    } else {
      missingMonths.push(monthName);
    }
  });
  if (!missingMonths.length || opts.indexOnly === true) return rows;
  var missingMap = {};
  missingMonths.forEach(function(monthName) { missingMap[monthName] = true; });
  var fallbackRows = loadTuitionFollowupRowsBundle_({ allowSheetFallback: false }).rows.filter(function(row) {
    return !!missingMap[String(row.monthName || "").trim()];
  });
  return mergeTuitionFollowupRows_(rows, fallbackRows);
}
...
function appendTuitionPaymentEntry(payload) {
  ...
  try {
    ...
    if (!paidAt) return { success: false, message: "납부일을 입력해 주세요." };
    if (!paymentType) return { success: false, message: "결재구분을 입력해 주세요." };
    ...
    var existingPayment = loadTuitionPaymentRequestFromFirestore_(requestId);
    if (existingPayment) {
      var duplicateIndexResult = updateTuitionPaymentReadIndexes_(existingPayment);
      var duplicateSnapshotUpdated = updateTuitionMonthSnapshotAfterPaymentWrite_(monthName, existingPayment);
      if (duplicateSnapshotUpdated) {
      ...
      return {
        success: true,
        duplicate: true,
        indexWarning: duplicateIndexResult && !duplicateIndexResult.success
          ? (duplicateIndexResult.message || "수납 인덱스 갱신이 지연되었습니다. 다음 새로고침에서 다시 확인해 주세요.")
          : "",
        snapshotWarning: duplicateSnapshotUpdated ? "" : "월별 요약 스냅샷이 갱신되지 않아 다음 조회 시 재계산됩니다.",
        storage: {
          firestore: true,
          sheetMirror: !!mirrorSheets,
          indexesUpdated: !!(duplicateIndexResult && duplicateIndexResult.success),
          snapshotUpdated: duplicateSnapshotUpdated,
          snapshotDeferred: !duplicateSnapshotUpdated
        },
    ...
    var indexResult = updateTuitionPaymentReadIndexes_(paymentRecord);
...
function getTuitionMonthlySalesOverview(payload) {
  try {
    ...
    var version = buildTuitionReportMonthsVersion_(months);
    var cached = readTuitionReportReadIndex_("monthly_sales", "global", version);
    if (cached) return cached;
    var rows = getTuitionPaymentRowsForMonthsFromIndexes_(months, { allowSheetFallback: false });
    var dueMap = {};
    var paidMap = {};
    var paidStudentMap = {};
    ...
    var result = {
      ...
      reportIndex: { source: "computed", version: version }
    };
    writeTuitionReportReadIndex_("monthly_sales", "global", result, version);
    return result;
  } catch (e) {
...
function getTuitionStudentMonthlyHistory(payload) {
  try {
    ...
    var version = buildTuitionReportMonthsVersion_(months);
    var cached = readTuitionReportReadIndex_("student_history", studentName, version);
    if (cached) return cached;

    var paymentRows = getTuitionPaymentRowsForMonthsFromIndexes_(months, { allowSheetFallback: false });
    var paymentByMonth = {};
    paymentRows.forEach(function(row) {
    ...
    var followupRows = getTuitionFollowupRowsForMonthsFromIndexes_(months, { allowSheetFallback: false });
    var followup = loadTuitionStudentFollowupByMonth_(studentName, followupRows);
    ...
    var result = { success: true, studentName: studentName, overview: overview, rows: rows, reportIndex: { source: "computed", version: version } };
    writeTuitionReportReadIndex_("student_history", studentName, result, version);
    return result;
  } catch (e) {
...
function getTuitionGuideDashboard(payload) {
  try {
    var months = getTuitionMonthSheetNames_({ allowSheetFallback: false });
    var version = buildTuitionReportMonthsVersion_(months);
    var cached = readTuitionReportReadIndex_("guide_dashboard", "global", version);
    if (cached) return cached;
    var monthMap = {};
    ...
    writeTuitionReportReadIndex_("guide_dashboard", "global", result, version);
    return result;
  } catch (e) {
...
function loadTuitionFollowupRowsBundle_(options) {
  var opts = options || {};
  var monthName = normalizeTuitionMonthName_(opts.monthName) || String(opts.monthName || "").trim();
  if (monthName) {
    var indexed = readTuitionFollowupMonthIndexBundle_(monthName);
    if (indexed.seeded) {
      return {
        source: "firestore-index",
        rows: indexed.rows,
        sheetRows: [],
        firestoreRows: indexed.rows,
  ...
  if (firestoreAvailable && monthName && isTuitionFollowupMonthFirestoreReady_(monthName)) {
    var monthRows = firestoreRows.filter(function(row) {
      return String(row && row.monthName || "").trim() === monthName;
    });
    writeTuitionFollowupMonthIndex_(monthName, monthRows, true);
    return {
      source: "firestore",
      rows: monthRows,
      sheetRows: [],
      firestoreRows: monthRows,
      skippedSheetRows: true
    };
  }
  if (opts.allowSheetFallback === false) {
    var firestoreOnlyRows = firestoreAvailable && monthName ? firestoreRows.filter(function(row) {
      return String(row && row.monthName || "").trim() === monthName;
    }) : (firestoreAvailable ? firestoreRows : []);
    if (monthName && firestoreOnlyRows.length) writeTuitionFollowupMonthIndex_(monthName, firestoreOnlyRows, true);
    return {
      source: firestoreAvailable ? "firestore-partial" : "firestore-error",
      rows: firestoreOnlyRows,
      sheetRows: [],
      firestoreRows: firestoreAvailable ? firestoreOnlyRows : null,
      skippedSheetRows: true
    };
  }
  ...
  if (monthName) {
    writeTuitionFollowupMonthIndex_(monthName, rows.filter(function(row) {
      return String(row && row.monthName || "").trim() === monthName;
    }), true);
  }
  return {
