...
test('tuition settlement exposes compact editing, payment methods, contact channels, and message settings', async () => {
  ...
  assert.match(source, /data-history-payment-key=/);
  assert.match(source, /data-lucide="history"/);
  assert.match(source, /id="tuitionPaymentHistoryModal"/);
  assert.match(source, /getTuitionPaymentHistory: true/);
  assert.match(source, /function openTuitionPaymentHistoryModal_\(paymentKey\)/);
  assert.match(source, /formatTuitionPaymentHistoryTime_/);
  assert.match(source, /event\.actorName/);
  assert.match(source, /event\.reason/);
  assert.match(source, /event\.previousPayment/);
  assert.match(source, /key: "dueDate", label: "청구일"/);
  assert.match(source, /document\.addEventListener\("keydown"/);
  assert.match(source, /event\.key === "Escape"/);
  assert.match(source, /event\.key !== "Tab"/);
  assert.match(source, /tuitionPaymentHistoryReturnFocusEl\.focus\(\)/);
  assert.match(source, /closeTuitionPaymentHistoryBtnEl\.focus\(\)/);
  assert.match(source, /response\.historyComplete === false/);
