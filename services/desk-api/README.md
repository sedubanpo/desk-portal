...
데스크 포털의 Apps Script 중계를 단계적으로 제거하기 위한 Cloud Run 서비스입니다. 2단계에서는 Firebase Auth 인증 경계 안에서 근무표, 일일 업무일지, 비품, 채용 업무 API를 제공합니다.
...
- `POST /v1/desk/:method`: 2단계 데스크 업무 API. 본문은 `{ "payload": { ... } }` 형식입니다.
...
쓰기 요청은 반드시 `x-idempotency-key`를 포함해야 하며, 동일 사용자·메서드·키의 완료 응답은 Firestore 영수증에서 재사용됩니다. 비품 수량 변경은 Realtime Database 트랜잭션으로 처리합니다.

## 안전 경계

- 수납·정산과 Google Calendar 연동은 아직 기존 경로를 사용합니다.
